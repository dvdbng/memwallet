...
func main () {
    ...
    if default_hash_pad_for_currency(currency) < 0 || passphrase == "" || salt == "" || len(fs.Args()) > 0 {
      usage()
    }


    if(*hash_pad == 0) {
      *hash_pad = default_hash_pad_for_currency(currency)
    }

    //fmt.Printf("Passphrase: %s\nSalt: %s\nHashPad: %d\n", passphrase, salt, hash_pad)
    result := warpwallet_secret(passphrase, salt, byte(*hash_pad))
    //fmt.Printf("Seed: %x\n", result)
