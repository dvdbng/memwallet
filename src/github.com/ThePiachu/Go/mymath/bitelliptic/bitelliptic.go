...
func (bc *BitCurve) Copy() *BitCurve {
	copiedCurve := new(BitCurve)
	copiedCurve.P = new(big.Int).Set(bc.P)
	copiedCurve.N = new(big.Int).Set(bc.N)
	copiedCurve.B = new(big.Int).Set(bc.B)
	copiedCurve.Gx = new(big.Int).Set(bc.Gx)
	copiedCurve.Gy = new(big.Int).Set(bc.Gy)
	copiedCurve.BitSize = bc.BitSize
	return copiedCurve
}
